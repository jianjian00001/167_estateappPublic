...
const {
data
} = await detail("weixiuleixing",id)
this.setData({
payAuth:utils.isAuthFront('weixiuleixing','支付')
})
...
const resUpdate = await update('weixiuleixing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','weixiuleixing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("weixiuleixing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
