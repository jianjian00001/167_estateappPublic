...
let res = {};
if(this.data.userid) {
res = await page(`weixiuleixing`, searchForm);
} else {
res = await list(`weixiuleixing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/weixiuleixing/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("weixiuleixing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/weixiuleixing/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/weixiuleixing/detail`
})
...
this.setData({
addAuth: utils.isAuth("weixiuleixing", "新增") || utils.isAuthFront("weixiuleixing", "新增"),
delAuth: utils.isAuth("weixiuleixing", "删除") || utils.isAuthFront("weixiuleixing", "删除"),
editAuth: utils.isAuth("weixiuleixing", "修改") || utils.isAuthFront("weixiuleixing", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("weixiuleixing",obj)
    }else{
         resList= await list("weixiuleixing",obj)
    }
