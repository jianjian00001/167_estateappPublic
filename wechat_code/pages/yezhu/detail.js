...
const {
data
} = await detail("yezhu",id)
this.setData({
payAuth:utils.isAuthFront('yezhu','支付')
})
...
const resUpdate = await update('yezhu', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','yezhu');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yezhu", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
