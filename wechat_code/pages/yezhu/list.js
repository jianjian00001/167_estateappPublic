...
if(this.data.yezhuzhanghao){
    searchForm['yezhuzhanghao'] = '%' + this.data.yezhuzhanghao + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`yezhu`, searchForm);
} else {
res = await list(`yezhu`, searchForm);
}
...
wx.navigateTo({
url: `/pages/yezhu/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yezhu",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/yezhu/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("yezhu", "新增") || utils.isAuthFront("yezhu", "新增"),
delAuth: utils.isAuth("yezhu", "删除") || utils.isAuthFront("yezhu", "删除"),
editAuth: utils.isAuth("yezhu", "修改") || utils.isAuthFront("yezhu", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("yezhu",obj)
    }else{
         resList= await list("yezhu",obj)
    }
