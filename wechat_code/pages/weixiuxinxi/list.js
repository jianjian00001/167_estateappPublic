...
let res = {};
if(this.data.userid) {
res = await page(`weixiuxinxi`, searchForm);
} else {
res = await list(`weixiuxinxi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/weixiuxinxi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("weixiuxinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/weixiuxinxi/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("weixiuxinxi", "新增") || utils.isAuthFront("weixiuxinxi", "新增"),
delAuth: utils.isAuth("weixiuxinxi", "删除") || utils.isAuthFront("weixiuxinxi", "删除"),
editAuth: utils.isAuth("weixiuxinxi", "修改") || utils.isAuthFront("weixiuxinxi", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("weixiuxinxi",obj)
    }else{
         resList= await list("weixiuxinxi",obj)
    }
