...
const {
data
} = await detail("weixiuxinxi",id)
this.setData({
payAuth:utils.isAuthFront('weixiuxinxi','支付')
})
...
const resUpdate = await update('weixiuxinxi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','weixiuxinxi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("weixiuxinxi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
