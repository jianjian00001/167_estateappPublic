...
async    onLoad(option) {
this.setData({
    weixiuxinxiShow: utils.isAuthFront('weixiushenqing','维修'),
})
...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.yezhuzhanghao,
...
const {
data
} = await detail("weixiushenqing",id)
this.setData({
payAuth:utils.isAuthFront('weixiushenqing','支付')
})
...
const resUpdate = await update('weixiushenqing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','weixiushenqing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("weixiushenqing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','weixiushenqing');
